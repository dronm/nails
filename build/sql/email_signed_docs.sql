-- Function: email_signed_docs(in_to_addr text, in_name_full text, in_key text)

-- DROP FUNCTION email_signed_docs(in_to_addr text, in_name_full text, in_key text);

-- Все функции по одному шаблону: возвращают структуру


CREATE OR REPLACE FUNCTION email_signed_docs(in_to_addr text, in_name_full text)
  RETURNS TABLE(
	from_addr text,
	from_name text,
	reply_name text,
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION email_signed_docs(in_to_addr text, in_name_full text) OWNER TO ;
