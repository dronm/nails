...
<!-- omit-xml-declaration="yes" html doctype-system="http://www.w3.org/TR/html4/strict.dtd"-->
...
<xsl:template name="initApp">
	...
	var application = new App_stroyserv({
		servVars:serv_vars
	...
	<xsl:if test="$LOGGED and (/document/model[@id='ServerVars']/row[1]/app_port or /document/model[@id='ServerVars']/row[1]/app_ports)">	
	//event server
...
<xsl:template name="modelFromTemplate">
	function showView(){
		...
		//
		//<xsl:value-of select="/document/model[@id=$m_id]" disable-output-escaping="no"/>
		//<xsl:value-of select="/document/model[@id=$m_id]" disable-output-escaping="yes"/>
...
<xsl:template match="model[@id='Script']/row">
	<!-- type="{type}" -->
	<script type="text/javascript" src="{concat(src,'?',$VERSION)}">
	</script>
</xsl:template>
...
<xsl:template name="page_header">
	...
	<div class="navbar navbar-inverse">
		...
		<div class="navbar-header">
			<a class="navbar-brand" href="/">Стройсервис
			</a>
		</div>
		</xsl:when>
		<xsl:otherwise>
